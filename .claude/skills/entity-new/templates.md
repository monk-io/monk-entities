...
export { action } from "monkec/base";  // Re-export for entity files to import
...
```ts
import { <Provider>Entity, <Provider>EntityDefinition, <Provider>EntityState, action } from "./<package>-base.ts";
import { Args } from "monkec/base";
import cli from "cli";
...
/**
 * @description <Provider> <Entity> entity.
 * <Brief description of what it creates/manages>.
 *
 * ## Required Permissions
 * - `<provider>:<Action1>` — <what for>
 * - `<provider>:<Action2>` — <what for>
 * - `cloudwatch:GetMetricStatistics` — cost estimation metrics
 *
 * ## Secrets
 * - Reads: none (authenticated via <provider> provider)
 * - Writes: none
 *
 * ## State Fields for Composition
 * - `state.id` - <Resource identifier, used by dependent entities>
 *
 * ## Composing with Other Entities
 * Works with:
 * - `<package>/<other-entity>` - <how they integrate>
 */
...
test-resource:
  ...
  permitted-secrets:
    <package>-api-token: true
  # Add services block if other entities need to read this entity's state
  services:
    data:
      protocol: custom

# Dependent entity — reads state from foundation via connection-target
test-dependent:
  defines: <package>/<dependent-class>
  parent_id: <- connection-target("parent") entity-state get-member("id")
  connections:
    parent:
      runnable: <package>-test/test-resource
      service: data
  depends:
    wait-for:
      runnables:
        - <package>-test/test-resource
      timeout: 120
```

**IMPORTANT**: `connection-target("name") entity-state get-member("field")` requires:
1. A `connections:` block mapping the name to `runnable` + `service: data`
2. The providing entity must have `services: { data: { protocol: custom } }`
3. A `depends.wait-for` to ensure the provider is ready before the dependent starts

Without all three, monk throws "could not access member value of error".
