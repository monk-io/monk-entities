...
Test files follow the naming convention `<feature>-template.yaml` + `<feature>-integration.test.yaml` — one pair per integration (group of related entities). For single-feature packages, files are named `stack-template.yaml` + `stack-integration.test.yaml`.

**Examples:**
- `cloud-tasks-template.yaml` + `cloud-tasks-integration.test.yaml`
- `identity-platform-template.yaml` + `identity-platform-integration.test.yaml`
- `stack-template.yaml` + `stack-integration.test.yaml` (single-feature packages)

Also check for `.env` or `env.example` — credentials for API-token based packages.
...
Read the relevant `*-integration.test.yaml` file(s) to understand:
...
**Run a specific integration test** (preferred — avoids running unrelated tests in the same package):
```bash
sudo INPUT_DIR=./src/<package>/ ./monkec.sh test --test-file <feature>-integration.test.yaml --verbose
```

Note: the `--test-file` value does NOT include the `test/` prefix — the runner prepends it automatically.

**Run all tests** in the package (only when needed):
```bash
