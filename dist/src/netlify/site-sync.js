...
// input/monk-entities/src/netlify/site.ts
const netlifyBase = require("monk-entities/netlify-base");
const NetlifyEntity = netlifyBase.NetlifyEntity;
const base = require("monkec/base");
const action = base.action;
const cli = require("cli");
var _listAccounts_dec, _restoreDeploy_dec, _getDeploy_dec, _createDeploy_dec, _listDeploys_dec, _getSite_dec, _a, _init;
var _Site = class _Site extends (_a = NetlifyEntity, _getSite_dec = [action("get-site")], _listDeploys_dec = [action("list-deploys")], _createDeploy_dec = [action("create-deploy")], _getDeploy_dec = [action("get-deploy")], _restoreDeploy_dec = [action("restore-deploy")], _listAccounts_dec = [action("list-accounts")], _a) {
  constructor() {
    super(...arguments);
  ...
  listAccounts() {
    ...
    cli.output(`Accounts for current user:`);
    if (accounts && Array.isArray(accounts)) {
      accounts.forEach((account, index) => {
        cli.output(`${index + 1}. ${account.name} (${account.slug})`);
        cli.output(`   Type: ${account.type}`);
        cli.output(`   Role: ${account.role}`);
        if (account.created_at) {
          cli.output(`   Created: ${account.created_at}`);
        }
        cli.output(``);
      });
    } else {
      cli.output(`No accounts found or unexpected response format`);
    }
  }
};
...
__decorateElement(_init, 1, "listAccounts", _listAccounts_dec, _Site);
