...
var _Domain = class _Domain extends (_a = ClerkEntity, _getInfo_dec = [action("get-info")], _a) {
  constructor() {
    super(...arguments);
  ...
  create() {
    if (this.definition.domain_id) {
      ...
      const domain = items.find((it) => it.id === this.definition.domain_id);
  ...
  getInfo() {
    ...
    const list = this.makeRequest("GET", "/domains");
    const items = Array.isArray(list?.data) ? list.data : [];
    const domain = items.find((it) => it.id === this.state?.domain_id);
    if (domain) {
...
__decorateElement(_init, 1, "getInfo", _getInfo_dec, _Domain);
