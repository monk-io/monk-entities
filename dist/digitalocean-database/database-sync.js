...
var _Database = class _Database extends (_a = DOProviderEntity, _getDatabase_dec = [action("getDatabase")], _listDatabases_dec = [action("listDatabases")], _createDatabase_dec = [action("createDatabase")], _deleteDatabase_dec = [action("deleteDatabase")], _getConnectionInfo_dec = [action("getConnectionInfo")], _resizeCluster_dec = [action("resizeCluster")], _getBackupInfo_dec = [action("get-backup-info")], _listBackups_dec = [action("list-backups")], _describeBackup_dec = [action("describe-backup")], _restore_dec = [action("restore")], _getRestoreStatus_dec = [action("get-restore-status")], _getCostEstimate_dec = [action("get-cost-estimate")], _costs_dec = [action("costs")], _a) {
  ...
  update() {
    ...
    const storageChanged = this.definition.storage_size_mib !== void 0 && this.definition.storage_size_mib !== this.state.storage_size_mib;
    if (sizeChanged || nodesChanged || storageChanged) {
      const resizeReq = {
        size: validateDatabaseSize(this.definition.size),
        num_nodes: this.definition.num_nodes
      };
      if (this.definition.storage_size_mib !== void 0) {
      ...
      cli.output(`\u{1F4CF} Resizing database cluster: ${changes.join(", ")}`);
      try {
  ...
  delete() {
    ...
    this.state.storage_size_mib = void 0;
    this.state.semantic_version = void 0;
  }
  checkReadiness() {
    if (!this.state.id) {
      return false;
...
\u23F3 Current status: ${db.status}`);
  ...
  getDatabasePricing() {
    ...
    const sizeInfo = _Database.DATABASE_PRICING[size];
    if (!sizeInfo) {
...
__publicField(_Database, "DATABASE_PRICING", {
