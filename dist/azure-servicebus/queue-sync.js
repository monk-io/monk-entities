...
// input/azure-servicebus/queue.ts
...
var _getRuntimeInfo_dec, _purgeMessages_dec, _getInfo_dec, _a, _init;
var _Queue = class _Queue extends (_a = AzureServiceBusEntity, _getInfo_dec = [action("get-info")], _purgeMessages_dec = [action("purge-messages")], _getRuntimeInfo_dec = [action("get-runtime-info")], _a) {
  constructor() {
    super(...arguments);
  ...
  create() {
    ...
    if (existsResult.resource) {
      ...
      const properties2 = existingQueue.properties;
      this.state = {
  ...
  getInfo(_args) {
    cli.output("==================================================");
    cli.output("\u{1F4E6} Service Bus Queue Information");
    cli.output("==================================================");
    const path = this.buildQueuePath(this.definition.namespace_name, this.definition.queue_name);
...
\u{1F4C5} Timestamps:`);
  ...
  purgeMessages(_args) {
    cli.output("==================================================");
    cli.output("\u{1F5D1}\uFE0F  Purging Queue Messages");
    cli.output("==================================================");
  ...
  getRuntimeInfo(_args) {
    cli.output("==================================================");
    cli.output("\u{1F4CA} Queue Runtime Information");
    cli.output("==================================================");
    const path = this.buildQueuePath(this.definition.namespace_name, this.definition.queue_name);
    const response = this.makeAzureRequest("GET", path);
    if (response.error) {
...
__decorateElement(_init, 1, "getInfo", _getInfo_dec, _Queue);
__decorateElement(_init, 1, "purgeMessages", _purgeMessages_dec, _Queue);
__decorateElement(_init, 1, "getRuntimeInfo", _getRuntimeInfo_dec, _Queue);
__decoratorMetadata(_init, _Queue);
__name(_Queue, "Queue");
var Queue = _Queue;
...
function main(def, state, ctx) {
  const entity = new Queue(def, state, ctx);
