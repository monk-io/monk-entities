...
var NeonEntity = class extends import_base.MonkEntity {
  ...
  makeRequest(method, path, body) {
    try {
      ...
      while (true) {
        ...
        lockedAttempts++;
        import_cli.default.output(`\u{1F512} Neon returned 423 (conflicting operations in progress); retrying ${lockedAttempts}/${maxLockedAttempts} in ${lockedDelayMs / 1e3}s...`);
        sleep(lockedDelayMs);
      }
      if (!response.ok) {
   ...
   */
  waitForOperation(project_id, operation_id, maxAttempts = 40, delayMs = 2e3) {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (operationData.operation) {
          if (operationData.operation.status === "finished" || operationData.operation.status === "completed") {
            import_cli.default.output(`\u2705 Operation ${operation_id} completed successfully`);
            return;
          }
      ...
      if (attempts < maxAttempts) {
        import_cli.default.output(`\u23F3 Waiting for operation ${operation_id} to complete... (attempt ${attempts}/${maxAttempts})`);
        sleep(delayMs);
      }
    }
    throw new Error(`Operation ${operation_id} did not complete within ${maxAttempts * delayMs / 1e3} seconds`);
  }
  /**
   * Helper method to wait for multiple operations to complete
   */
  waitForOperations(project_id, operation_ids, maxAttempts = 40, delayMs = 2e3) {
    if (!operation_ids || operation_ids.length === 0) return;
    import_cli.default.output(`\u23F3 Waiting for ${operation_ids.length} operations to complete...`);
    for (const operation_id of operation_ids) {
      this.waitForOperation(project_id, operation_id, maxAttempts, delayMs);
    }
  }
};
__publicField(NeonEntity, "readiness", { period: 15, initialDelay: 2, attempts: 20 });
