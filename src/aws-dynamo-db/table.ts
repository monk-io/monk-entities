...
export class DynamoDBTable extends AWSDynamoDBEntity<DynamoDBTableDefinition, DynamoDBTableState> {
     ...
     */
    private fetchDynamoDBPricing(): {
        readCapacityUnit: number;
        writeCapacityUnit: number;
        onDemandReadUnit: number;
        onDemandWriteUnit: number;
        ...
        // Fetch provisioned capacity pricing
     ...
     */
    private getCloudWatchDynamoDBMetrics(): {
    ...
    /**
     * Get detailed cost estimate for the DynamoDB table
     */
    ...
    getCostEstimate(): void {
        ...
        cli.output(`\n💰 Cost Estimate for DynamoDB Table: ${this.state.table_name}`);
        ...
        if (billingMode === 'PAY_PER_REQUEST') {
            // On-demand mode - use CloudWatch metrics for actual usage
