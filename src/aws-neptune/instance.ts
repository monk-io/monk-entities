...
export class Instance extends AWSNeptuneEntity<InstanceDefinition, InstanceState> {
    ...
    getCostEstimate(): void {
        ...
        // Storage and I/O are cluster-level costs billed once per cluster, not per instance.
        // Only the writer instance includes them to avoid double-counting when costs are
        // aggregated across all instances in the cluster.
        ...
        if (isWriter) {
            if (metrics && metrics.volumeBytesUsed > 0) {
                ...
                cli.output(`\n💾 Storage Costs (cluster-level, writer only):`);
            ...
            cli.output(`\n📈 I/O Costs (cluster-level, writer only):`);
    ...
    costs(): void {
        ...
        try {
            ...
            // Storage and I/O are cluster-level costs billed once per cluster.
            // Only include them on the writer instance to avoid double-counting when
            // costs() results are aggregated across all instances in the cluster.
            const isWriter = this.state.is_cluster_writer === true;
            if (isWriter) {
