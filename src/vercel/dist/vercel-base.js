...
// input/vercel/vercelBase.ts
var vercel_base_exports = {};
__export(vercel_base_exports, {
  VercelEntity: () => VercelEntity
});
module.exports = __toCommonJS(vercel_base_exports);
...
var VercelEntity = class extends import_base.MonkEntity {
  constructor() {
    super(...arguments);
    /**
     ...
     */
    __publicField(this, "baseUrl", "https://api.vercel.com");
  }
  /**
   ...
   */
  start() {
    import_cli.default.output(`Starting Vercel operations for: ${this.getEntityName()}`);
  }
  /**
   * Standard stop implementation for Vercel entities
   */
  stop() {
    import_cli.default.output(`Stopping Vercel operations for: ${this.getEntityName()}`);
  }
  /**
  ...
  deleteResource(path, resourceName) {
    ...
    try {
      ...
      if (errorMessage.includes("500") || errorMessage.includes("unexpected response code 500")) {
        ...
        import_cli.default.output(`The resource will be cleaned up by Vercel automatically`);
  ...
  waitForOperation(operationId, maxAttempts = 40, delayMs = 2e3) {
    ...
    throw new Error(`Operation ${operationId} did not complete within the expected time`);
  }
  /**
...
__publicField(VercelEntity, "readiness", { period: 15, initialDelay: 2, attempts: 20 });
